...
class MathVerifyScorer:
    """Scores model outputs against ground truth using math-verify.

    Instead of going through ``math_metric`` (which wraps ``verify`` with a
    hard-coded 5 s ``signal.alarm`` that *overrides* our own per-item alarm),
    we call ``parse`` + ``verify`` directly and forward ``per_item_timeout_s``
    as ``timeout_seconds`` so that the inner timeout is consistent with the
    outer one.
    """

    def __init__(self):
        self._gold_extraction_target = (LatexExtractionConfig(),)
        self._pred_extraction_target = (ExprExtractionConfig(), LatexExtractionConfig())
    ...
    def compute_score(
        ...
        # Hard per-item timeout via signal (acts as a safety net)
        signal.signal(signal.SIGALRM, _alarm_handler)
        signal.alarm(per_item_timeout_s + 1)  # +1s grace for the inner timeout
        try:
            extracted_golds = [parse(gt_boxed, self._gold_extraction_target)]
            extracted_preds = [parse(model_output, self._pred_extraction_target)]

            # If we couldn't extract any prediction, score is 0
            if all(len(p) == 0 for p in extracted_preds):
                return 0.0

            # Replicate math_metric aggregation: max over preds, any over golds
            score = max(
                (
                    1.0
                    if any(
                        verify(gold, pred, timeout_seconds=per_item_timeout_s)
...
class MultiThreadNaiveRewardManager:
    ...
    @staticmethod
    def _is_math_data_source(data_source: str) -> bool:
        """Return True if data_source should be scored by MathVerifyScorer (Ray actors).

        Non-math sources (e.g. sciknow MCQ, search QA) are scored directly via
        the compute_score function passed at init time.
        """
        # These prefixes/names are handled by specialized (non-math-verify) scorers
        _NON_MATH_PREFIXES = ("sciknow", "searchR1_", "maze", "mmlu-pro")
        _NON_MATH_EXACT = {"countdown", "hiyouga/geometry3k"}
        if any(data_source.startswith(p) for p in _NON_MATH_PREFIXES):
            return False
        if data_source in _NON_MATH_EXACT:
            return False
        return True
    ...
    def __call__(self, data: DataProto, return_dict: bool = False):
        ...
        # ---------------------------------------------------------
        # Split items: non-math items scored directly via compute_score,
        # math items sent to parallel Ray actors (MathVerifyScorer).
        # ---------------------------------------------------------
        results: Dict[int, Dict[str, float]] = {}

        # Score non-math items directly (e.g. sciknow MCQ)
        math_indices: List[int] = []
        for idx, info in enumerate(items):
            ds = info["data_source"]
            if self._is_math_data_source(ds):
                math_indices.append(idx)
            else:
                score = self.compute_score(
                    data_source=ds,
                    solution_str=info["response"],
                    ground_truth=info["ground_truth"],
                )
                if isinstance(score, dict):
                    reward = float(score.get("score", 0.0))
                    acc = float(score.get("accuracy", reward))
                else:
                    reward = float(score)
                    acc = reward
                results[idx] = {"score": reward, "accuracy": acc}

        # Send math items to parallel Ray actors
        pending: Set[ray.ObjectRef] = set()
        start_time: Dict[ray.ObjectRef, float] = {}
        ref_to_batch: Dict[ray.ObjectRef, List[int]] = {}

        math_pos = 0  # pointer into math_indices

        def submit_one_batch():
            nonlocal math_pos
            if math_pos >= len(math_indices):
                return

            batch: List[int] = []
            while math_pos < len(math_indices) and len(batch) < self._batch_size:
                batch.append(math_indices[math_pos])
                math_pos += 1
        ...
        # prime inflight
        while len(pending) < self._max_inflight_batches and math_pos < len(math_indices):
            submit_one_batch()
