...
class ActorRolloutRefWorker(Worker, WorkerProfilerExtension):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_probe_forward_logprobs(self, prompts: DataProto):
        """Compute answer-token logprobs via a single vLLM forward pass.
        Used by the faithfulness probe in 'forward' mode.
        """
        prompts = prompts.to(get_device_id())
        ...
        with self.rollout_sharding_manager:
            log_gpu_memory_usage("After entering rollout sharding manager (probe forward)", logger=logger)

            prompts = self.rollout_sharding_manager.preprocess_data(prompts)
            output = self.rollout.compute_probe_forward_logprobs(prompts=prompts)

            log_gpu_memory_usage("After probe forward logprobs", logger=logger)
        ...
        # clear kv cache
        get_torch_device().empty_cache()
        log_gpu_memory_usage("After probe forward cleanup", logger=logger)
        return output
