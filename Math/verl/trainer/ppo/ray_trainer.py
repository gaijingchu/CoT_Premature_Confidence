...
from verl.utils.checkpoint.checkpoint_manager import BaseCheckpointManager, find_latest_ckpt_path, _find_all_ckpt_steps, is_checkpoint_valid
...
def _find_cot_boundaries(
    ...
    * **cot_end** – index of the first ``boxed`` token (if found),
      otherwise the full response length.
    * **boundary_tag** – human-readable label for debugging.

    Note: We search for the single 'boxed' token (79075 for Qwen3) instead of
    the full ``\\boxed{`` sequence because the tokenization of ``\\`` is
    context-dependent (e.g., " \\boxed{" tokenizes differently than "\\boxed{").
    """
    vrl = len(valid_response)
    cot_start = 0

    # Find first occurrence of the 'boxed' token
    boxed_pos = -1
    for i in range(vrl):
        if valid_response[i].item() == boxed_token_id:
            # Step back over any preceding backslash token(s) that are part of \boxed
            # Common patterns:
            #   "\" (token 59), " \" (token 1124), "$\" (token 57960)
            # These are context-dependent tokenizations of backslash before 'boxed'
            pos = i
            while pos > 0 and valid_response[pos - 1].item() in (59, 1124, 57960):
                pos -= 1
            boxed_pos = pos
            break
...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if val_gen_batch_size and val_gen_batch_size > 0 and val_n > val_gen_batch_size:
                ...
                total_seqs = len(test_batch)
                print(f"[Validation] Generating {num_prompts} prompts × {val_n} rollouts = {total_seqs} sequences")
                ...
                print(f"[Validation] Generation started ({total_seqs} sequences)...")
                if not self.async_rollout_mode:
                    ...
                    self.async_rollout_manager.sleep()
                print(f"[Validation] Generation finished in {_time.time() - _gen_start:.1f}s")
            ...
            _reward_start = _time.time()
            print(f"[Validation] Computing rewards for {len(test_batch)} samples...")
            ...
            print(f"[Validation] Rewards computed in {_time.time() - _reward_start:.1f}s, mean_reward={mean_score:.4f}")
    ...
    def _load_checkpoint(self):
        ...
        # Build ordered list of candidates to try: the selected one first, then
        # earlier checkpoints in descending step order.
        if self.config.trainer.resume_mode == "auto":
            current_step = int(global_step_folder.split("global_step_")[-1])
            all_steps = _find_all_ckpt_steps(checkpoint_folder)
            # candidates = current checkpoint, then all earlier ones
            candidates = [global_step_folder]
            for s in all_steps:
                p = os.path.join(checkpoint_folder, f"global_step_{s}")
                if s < current_step and is_checkpoint_valid(p):
                    candidates.append(p)
        else:
            candidates = [global_step_folder]
        ...
        for candidate_folder in candidates:
            print(f"Attempting to load checkpoint: {candidate_folder}")
            step = int(candidate_folder.split("global_step_")[-1])
            actor_path = os.path.join(candidate_folder, "actor")
            critic_path = os.path.join(candidate_folder, "critic")
            try:
                ...
                print(f"Successfully resumed from {candidate_folder}")
                loaded = True
                break
            except Exception as e:
                print(f"WARNING: Failed to load checkpoint {candidate_folder}: {e}")
                print("Trying next earlier checkpoint...")
                continue

        if not loaded:
            print("WARNING: All checkpoints failed to load. Training from scratch.")
            return 0

        # load dataloader,
        # TODO: from remote not implemented yet
        dataloader_local_path = os.path.join(candidate_folder, "data.pt")
    ...
    def _run_probe_forward_single_batch(
        self,
        sub_batch: DataProto,
        batch_start_idx: int,
        probe_cfg,
        ...
        """Run probe in **forward** mode on a single sub-batch.

        Instead of MC decode sampling, we construct the full answer sequence
        ``[prompt] + [truncated CoT] + [suffix] + [answer_tokens]`` and do a
        single forward pass to get ``P(correct_answer | prefix)`` from the
        model's own logprobs.

        Returns:
            probe_scores – Tensor of shape ``(sub_batch_size, num_truncations + 1)``.
                Each value is ``exp(sum_logprob)`` ∈ [0, 1], representing the
                probability that the model generates the exact correct answer.
        """
        import time as _time
        ...
        # Clamp out-of-vocabulary token IDs (same as decode mode)
        max_valid_token_id = max(
            max(tokenizer.get_vocab().values()),
            tokenizer.vocab_size - 1,
        )

        # Pre-collect per-sample ground truths
        ...
        # ── 2. Find CoT boundaries & build probe sequences with answer ──
        all_probe_token_ids = []     # full sequences (prefix + suffix + answer)
        answer_start_positions = []  # where the answer tokens begin (unpadded)
        ...
        for i in range(sub_batch_size):
            ...
            cot_start, cot_end, boundary_tag = _find_cot_boundaries(
                valid_response, boxed_token_id
            )
            cot_len = cot_end - cot_start

            ground_truth = sample_ground_truths[i]

            # Tokenize the full tail: suffix + answer + "}"
            # We tokenize together so that the token boundaries at \boxed{ANSWER}
            # are the most natural for the model's BPE vocabulary.
            full_tail_str = suffix_str + str(ground_truth) + "}"
            full_tail_ids = tokenizer.encode(full_tail_str, add_special_tokens=False)
            full_tail_ids = torch.tensor(full_tail_ids, dtype=torch.long)

            # answer_start_in_tail: the suffix_ids length gives us the approximate
            # boundary; answer tokens begin right after the suffix tokens.
            ...
            for k in range(num_trunc_points):
                ...
                # Full sequence: [prompt] + [truncated_cot] + [suffix + answer + "}"]
                probe_tokens = torch.cat(
                    [valid_prompt, truncated_response, full_tail_ids], dim=0
                )
                all_probe_token_ids.append(probe_tokens)

                # Answer starts in the unpadded sequence
                answer_start = len(valid_prompt) + len(truncated_response) + suffix_len
        ...
        # ── 4. Package into DataProto and send to vLLM for forward logprobs ──
        # IMPORTANT: answer_start_positions is stored as a batch tensor (not meta_info)
        # so that DataProto.chunk() correctly splits it across distributed workers.
        probe_batch = TensorDict(
            {
                "input_ids": padded_input_ids,
                "attention_mask": probe_attention_mask,
                "position_ids": probe_position_ids,
                "answer_start_positions": torch.tensor(
                    answer_start_positions, dtype=torch.long
                ),
            },
        ...
        for i in range(sub_batch_size):
            for k in range(num_trunc_points):
                ...
                # P(answer | prefix) = exp(sum_logprob)
                prob = torch.exp(logprobs[probe_idx]).clamp(0.0, 1.0).item()
                probe_scores[i, k] = prob

        timing_raw["probe_reward"] = timing_raw.get("probe_reward", 0) + 0.0

        del probe_output, logprobs
        torch.cuda.empty_cache()
    ...
    def _run_probe(
        ...
        """Run the faithfulness probe (forward / boxed mode).

        For each response we:
          1. Locate the CoT boundary at the first ``\\boxed{`` token.
          2. Truncate the CoT at fractions ``0/N``, ``1/N``, ..., ``N/N``.
          3. Build ``[prompt] + [truncated CoT] + suffix + ground_truth + "}"``.
          4. Compute ``P(answer | prefix)`` via a single vLLM forward pass over
             the appended ground-truth tokens.

        Returns ``(batch_size, num_truncations + 1)`` probabilities. ``num_splits``
        chunks the batch to fit in GPU memory.
        """
        ...
        suffix_ids = torch.tensor(
            tokenizer.encode(probe_cfg.suffix, add_special_tokens=False),
            dtype=torch.long,
        )

        boxed_sample = tokenizer.encode("\\boxed{", add_special_tokens=False)
        boxed_token_id = boxed_sample[1] if len(boxed_sample) >= 2 else boxed_sample[0]

        def _run_one(sub_batch, start_idx):
            return self._run_probe_forward_single_batch(
                sub_batch, start_idx, probe_cfg, tokenizer, pad_token_id,
                suffix_ids, boxed_token_id, timing_raw,
            )
        ...
        all_probe_scores = []
        for split_idx, sub_batch in enumerate(sub_batches):
            all_probe_scores.append(_run_one(sub_batch, split_idx * sub_batch_size))
            torch.cuda.empty_cache()
    ...
    def fit(self):
        ...
        # Resume wandb run if a previous run ID was loaded from checkpoint.
        # When self.wandb_id is not None, Tracking will call wandb.init(resume="must")
        # to continue logging on the same run (useful for SLURM preemption recovery).
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # ── Overconfidence penalty on advantage (linspace-weighted) ──
                    # Penalty is the linspace(+0.5..-0.5)-weighted sum of probe
                    # pass-rates across truncation points; this corresponds to
                    # the `penalty_mode=early_mean` path that the wandb Math
                    # reference runs actually executed (the config key name is
                    # historical; the formula is linspace, not a plain mean).
                    #
                    #   early-high + late-low  → positive penalty (punish overconf)
                    #   early-low  + late-high → negative penalty (reward faithful CoT)
                    #
                    # Use batch.batch["probe_scores"] (not the local var) because
                    # _balance_batch may have re-ordered the batch in place.
                    if probe_scores is not None:
                        ...
                        if overconf_coeff > 0.0:
                            ...
                            num_trunc_points = reordered_probe_scores.shape[1]
                            overconf_weights = torch.linspace(0.5, -0.5, num_trunc_points)
                            penalty = overconf_coeff * (
                                reordered_probe_scores
                                * overconf_weights.unsqueeze(0).to(reordered_probe_scores.device)
                            ).sum(dim=1)
                            ...
                            metrics["probe/overconf_penalty_abs_mean"] = penalty.abs().mean().item()
                ...
                if probe_scores is not None:
                    ...
                    if "token_level_scores" in batch.batch.keys():
                        ...
                        def _log_probe_group(prefix, scores):
                            ...
                            metrics[f"{prefix}/overconf_dot"] = oc.mean().item()
