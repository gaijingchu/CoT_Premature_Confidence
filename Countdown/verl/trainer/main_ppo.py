...
def _pool_compute_probe_score(args):
    """Worker function for probe scoring, runnable in a spawned subprocess.

    Re-imports the reward modules inside the worker so the function is
    picklable across the ``spawn`` context. Uses ``format_score=0`` so only
    fully-correct answers count toward the probe pass-rate.
    """
...
def main_task(config):
    ...
    # Probe scoring runs many independent reward calls per step; parallelize via a
    # shared multiprocessing pool. The same pool object is handed to the trainer.
    probe_pool = None
    probe_cfg = config.get('probe', {})
    if probe_cfg.get('enable', False):
        num_workers = probe_cfg.get('num_workers', 4)
        probe_pool = multiprocessing.get_context('spawn').Pool(processes=num_workers)
