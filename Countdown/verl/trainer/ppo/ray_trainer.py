...
# Probe helpers (module-level for testability — see tests/test_probe_boundaries.py).

def find_token_seq(tokens: 'torch.Tensor', pattern: 'torch.Tensor') -> int:
    """Return the index of the first occurrence of ``pattern`` in 1-D ``tokens``, or -1."""
    plen = len(pattern)
...
def find_cot_boundaries(valid_response, think_open_ids, think_close_ids, answer_open_ids):
    """Locate the chain-of-thought span inside a response.

    Returns ``(cot_start, cot_end, boundary_tag)``. The end boundary is chosen
    by priority: ``</think>`` -> ``<answer>`` (fallback when the prompt ends
    with ``<think>`` and the model never re-closes it) -> end of response.
    """
    think_s = find_token_seq(valid_response, think_open_ids)
    cot_start = think_s + len(think_open_ids) if think_s >= 0 else 0

    think_e = find_token_seq(valid_response, think_close_ids)
    if think_e >= 0:
        return cot_start, think_e, '</think>'

    answer_s = find_token_seq(valid_response, answer_open_ids)
    if answer_s >= 0:
        return cot_start, answer_s, '<answer>(fallback)'

    return cot_start, len(valid_response), 'none(full response)'
...
class RayPPOTrainer(object):
    ...
    def __init__(self,
        ...
        # Optional multiprocessing pool used to parallelize probe scoring.
        probe_cfg = config.get('probe', {})
        self._score_pool = shared_pool
        self._score_pool_timeout = probe_cfg.get('timeout', 120)
    ...
    def _run_probe_single_batch(self, sub_batch: DataProto, batch_start_idx: int,
        ...
        """Probe a single (sub-)batch. Returns probe_scores of shape (B, num_truncations + 1)."""
        ...
        # Build probe prompts: [valid_prompt, response[:cot_start+trunc], suffix] for each truncation k.
        all_probe_token_ids = []
        for i in range(sub_batch_size):
            ...
            cot_start, cot_end, _ = find_cot_boundaries(
            ...
            for k in range(num_trunc_points):
                ...
                probe_tokens = torch.cat(
                    [valid_prompt, valid_response[:trunc_end], suffix_ids], dim=0)
                all_probe_token_ids.append(probe_tokens)

        # Left-pad to a uniform length and build attention_mask / position_ids.
        ...
        with _timer('probe_reward', timing_raw):
            ...
            scoring_args = []  # (data_source, solution_str, ground_truth) per (sample, trunc, mc)
            for i in range(sub_batch_size):
                ...
                for k in range(num_trunc_points):
                    ...
                    for m_offset in range(mc_samples):
                        ...
                        valid_gen_ids = gen_ids[gen_ids != pad_token_id]
                        gen_str = tokenizer.decode(valid_gen_ids, skip_special_tokens=False)
                        # The suffix is added to the prompt during generation, but the reward
                        # function expects the full assistant turn. Re-prepend it here so the
                        # extractor sees a well-formed <answer>...</answer>.
                        clean_solution = f"<|im_start|>assistant\n{suffix_str}{gen_str}"
                        scoring_args.append((data_source, clean_solution, ground_truth))

            total_tasks = len(scoring_args)
            if self._score_pool is not None:
                from verl.trainer.main_ppo import _pool_compute_probe_score
                chunksize = max(1, total_tasks // 64)
                try:
            ...
            # Aggregate to pass-rate per (sample, trunc).
            score_idx = 0
            for i in range(sub_batch_size):
                for k in range(num_trunc_points):
                    chunk = all_scores[score_idx:score_idx + mc_samples]
                    score_idx += mc_samples
                    probe_scores[i, k] = sum(1 for s in chunk if s >= 1.0) / mc_samples
    ...
    def _run_probe(self, batch: DataProto, timing_raw: Dict[str, float]) -> torch.Tensor:
        """Run the faithfulness probe.

        For each response, truncate its chain-of-thought at fractions
        0/N, 1/N, ..., N/N, append a fixed suffix that asks the model to
        produce its final answer immediately, generate ``mc_samples`` MC
        completions, and report the fraction that match the ground truth.

        The CoT boundary is found in priority order:
          1. tokens between ``<think>`` and ``</think>`` (standard case);
          2. tokens before ``<answer>`` (fallback when the prompt already
             ends with ``<think>`` and the response never re-closes it);
          3. the entire response (last resort).

        Returns a tensor of shape ``(batch_size, num_truncations + 1)``.
        ``num_splits`` allows splitting the batch into equal chunks to fit
        in GPU memory.
        """
        ...
        suffix_ids = torch.tensor(
            tokenizer.encode(probe_cfg.suffix, add_special_tokens=False), dtype=torch.long)
        ...
        for split_idx, sub_batch in enumerate(sub_batches):
            ...
            all_probe_scores.append(self._run_probe_single_batch(
                sub_batch, start_idx, probe_cfg, tokenizer, pad_token_id,
                suffix_ids, think_open_ids, think_close_ids, answer_open_ids,
                timing_raw))
            torch.cuda.empty_cache()

        return torch.cat(all_probe_scores, dim=0)
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if 'probe_scores' in batch.batch.keys():
                            ...
                            # ── Overconfidence penalty on reward (before GRPO normalization) ──
                            # Subtracted at the last valid token of each sample so it enters
                            # `token_level_scores` and gets group-normalised by GRPO advantage
                            # computation (i.e. divided by the per-group reward std).
                            ...
                            if overconf_coeff != 0.0:
                                ...
                                attention_mask = batch.batch['attention_mask']
                                response_mask = attention_mask[:, -response_length:]
                                last_valid_idx = response_mask.sum(dim=-1).long() - 1
